...
function inferType(node, env) {
    ...
    switch (node.type) {
        ...
        case 'CallExpression': {
            // if call to a typed function, return its declared return type
            if (node.callee && node.callee.type === 'Identifier') {
                const fname = node.callee.name;
                // built-in fetch helper returns Promise<any>
                if (fname === 'fetch') return 'Promise<any>';
                if (env && env.__typesSpec && env.__typesSpec.funcs && env.__typesSpec.funcs[fname]) {
                    return env.__typesSpec.funcs[fname].returns || 'any';
                }
            }
            return 'any';
        }
...
function validate(ast, typesSpec) {
    ...
    // attach typesSpec for inferType to consult
    globalEnv.__typesSpec = typesSpec || {};

    // collect declared functions from AST so we can check calls to them even without .shari
    const declaredFuncs = {};
    if (ast && ast.type === 'Program') {
        for (const s of ast.body) {
            if (s && s.type === 'FunctionDeclaration' && s.name) declaredFuncs[s.name] = s;
        }
    }
    ...
    function checkExpression(expr, envLocal, errors, warnings) {
        ...
        switch (expr.type) {
            ...
            case 'Literal':
                break;
            case 'CallExpression': {
                // check call expression against typesSpec if available
                // callee may be Identifier or member expression; we handle Identifier
                if (expr.callee && expr.callee.type === 'Identifier') {
                    const fname = expr.callee.name;
                    const spec = typesSpec && typesSpec.funcs && typesSpec.funcs[fname];
                    if (!spec) {
                        // if function declared in source, we can still check arg count
                        if (declaredFuncs[fname]) {
                            const expectedLen = (declaredFuncs[fname].params || []).length;
                            const got = (expr.arguments || []).length;
                            if (expectedLen !== got) {
                                errors.push(`Function call '${fname}' argument count mismatch: expected ${expectedLen} but got ${got}`);
                            }
                        } else {
                            warnings.push(`Call to undeclared function '${fname}' (assumed any)`);
                        }
                    } else {
                        const expected = Object.keys(spec.params || {});
                        const got = (expr.arguments || []).length;
                        if (expected.length !== got) {
                            errors.push(`Function call '${fname}' argument count mismatch: expected ${expected.length} but got ${got}`);
                        }
                        // check argument types
                        (expr.arguments || []).forEach((a, idx) => {
                            const at = inferType(a, envLocal);
                            const pname = expected[idx];
                            const ptype = pname ? spec.params[pname] : null;
                            if (ptype && ptype !== 'any' && !typeAllows(ptype, at)) {
                                errors.push(`Argument ${idx+1} for function '${fname}' expected '${ptype}' but got '${at}'`);
                            }
                        });
                    }
                }
                // check args
                (expr.arguments || []).forEach(a => checkExpression(a, envLocal, errors, warnings));
                break;
            }
            default:
                break;

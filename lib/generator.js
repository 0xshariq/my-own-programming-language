    ...
    function gen(n) {
        ...
        switch (n.type) {
            ...
            case 'Identifier':
                return n.name;
            case 'CallExpression':
                // Special-case fetch wrapper: calls to the `fetch` helper (produced by `lao`) should
                // automatically parse JSON via .then(r => r.json()). This keeps usage simple in the
                // source language: `pratiksha lao(url)` will return the parsed JSON.
                if (n.callee && n.callee.type === 'Identifier' && n.callee.name === 'fetch') {
                    return `fetch(${(n.arguments || []).map(a => gen(a)).join(',')}).then(r => r.json())`;
                }

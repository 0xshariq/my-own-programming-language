    ...
    function gen(n) {
        ...
        switch (n.type) {
            ...
            case 'MemberExpression':
                return `${gen(n.object)}.${gen(n.property)}`;
            case 'CallExpression':
                // Special-case fetch helper calls (produced by `lao`/`mangao`):
                // emit direct fetch(...).then(r => r.json()) so no runtime helper is required.
                if (n.callee && n.callee.type === 'Identifier' && n.callee.name === 'fetch') {
                    const args = (n.arguments || []).map(a => gen(a)).join(',');
